...
// OIDCAuthConfig represents OpenID Connect settings for Bifrost dashboard SSO.
...
// NormalizedScopes returns OIDC scopes with a safe default for login.
func (c *OIDCAuthConfig) NormalizedScopes() []string {
...
// IsConfigured reports whether enough OIDC metadata exists to begin login.
func (c *OIDCAuthConfig) IsConfigured() bool {
	return c != nil &&
		strings.TrimSpace(c.IssuerURL) != "" &&
		strings.TrimSpace(c.ClientID) != "" &&
		c.ClientSecret != nil &&
		strings.TrimSpace(c.ClientSecret.GetValue()) != ""
}

// FullyRedacted returns a copy safe for API responses.
func (c *OIDCAuthConfig) FullyRedacted() *OIDCAuthConfig {
	if c == nil {
		return nil
	}
	redacted := *c
	redacted.Scopes = slices.Clone(c.Scopes)
	redacted.AllowedEmailDomains = slices.Clone(c.AllowedEmailDomains)
	redacted.AllowedEmails = slices.Clone(c.AllowedEmails)
	redacted.ClientSecret = c.ClientSecret.FullyRedacted()
	return &redacted
}
...
// EffectiveAuthType returns the dashboard auth mode while preserving password
// as the default for existing configs that only set is_enabled.
func (c *AuthConfig) EffectiveAuthType() string {
