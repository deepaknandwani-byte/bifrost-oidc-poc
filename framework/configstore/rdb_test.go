...
func TestRDBConfigStore_AuthConfigOIDCRoundTripPreservesSecretRef(t *testing.T) {
	store := setupRDBTestStore(t)
	ctx := context.Background()
	t.Setenv("BIFROST_TEST_OIDC_CLIENT_SECRET", "resolved-client-secret")

	require.NoError(t, store.UpdateAuthConfig(ctx, &AuthConfig{
		IsEnabled: true,
		AuthType:  AuthTypeOIDC,
		OIDC: &OIDCAuthConfig{
			IssuerURL:           "https://issuer.example.com/oauth2/default",
			ClientID:            "client-id",
			ClientSecret:        schemas.NewSecretVar("env.BIFROST_TEST_OIDC_CLIENT_SECRET"),
			RedirectURL:         "https://bifrost.example.com/api/session/sso/callback",
			Scopes:              []string{"email"},
			AllowedEmailDomains: []string{"thoughtspot.com"},
			AllowedEmails:       []string{"deepak@thoughtspot.com"},
		},
	}))

	fetched, err := store.GetAuthConfig(ctx)
	require.NoError(t, err)
	require.NotNil(t, fetched)
	require.NotNil(t, fetched.OIDC)
	assert.True(t, fetched.IsEnabled)
	assert.Equal(t, AuthTypeOIDC, fetched.EffectiveAuthType())
	assert.Equal(t, "https://issuer.example.com/oauth2/default", fetched.OIDC.IssuerURL)
	assert.Equal(t, "client-id", fetched.OIDC.ClientID)
	assert.Equal(t, "resolved-client-secret", fetched.OIDC.ClientSecret.GetValue())
	assert.Equal(t, "env.BIFROST_TEST_OIDC_CLIENT_SECRET", fetched.OIDC.ClientSecret.GetRawRef())
	assert.Equal(t, []string{"openid", "email"}, fetched.OIDC.NormalizedScopes())

	var row tables.TableGovernanceConfig
	require.NoError(t, store.DB().WithContext(ctx).First(&row, "key = ?", tables.ConfigOIDCAuthKey).Error)
	assert.Contains(t, row.Value, "env.BIFROST_TEST_OIDC_CLIENT_SECRET")
	assert.NotContains(t, row.Value, "resolved-client-secret")
}
