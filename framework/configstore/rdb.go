...
func secretVarToStoredString(value *schemas.SecretVar) string {
	if value == nil {
		return ""
	}
	if value.IsFromSecret() {
		return value.GetRawRef()
	}
	return value.GetValue()
}

type storedOIDCAuthConfig struct {
...
func (s *RDBConfigStore) GetAuthConfig(ctx context.Context) (*AuthConfig, error) {
	...
	if err := s.DB().WithContext(ctx).First(&tables.TableGovernanceConfig{}, "key = ?", tables.ConfigAuthTypeKey).Select("value").Scan(&authType).Error; err != nil {
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, err
		}
	}
	if err := s.DB().WithContext(ctx).First(&tables.TableGovernanceConfig{}, "key = ?", tables.ConfigOIDCAuthKey).Select("value").Scan(&oidcConfigJSON).Error; err != nil {
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, err
		}
	}
	if username == nil && password == nil && authType == nil && oidcConfigJSON == nil {
		return nil, nil
	}
	resolvedAuthType := AuthTypePassword
	if authType != nil && strings.TrimSpace(*authType) != "" {
		resolvedAuthType = strings.TrimSpace(*authType)
	}
	var oidcConfig *OIDCAuthConfig
	if oidcConfigJSON != nil && strings.TrimSpace(*oidcConfigJSON) != "" {
		var parsed OIDCAuthConfig
		if err := json.Unmarshal([]byte(*oidcConfigJSON), &parsed); err != nil {
			return nil, fmt.Errorf("failed to unmarshal OIDC auth config: %w", err)
		}
		oidcConfig = &parsed
	}
	usernameSecret := &schemas.SecretVar{}
	if username != nil {
		usernameSecret = schemas.NewSecretVar(*username)
	}
	passwordSecret := &schemas.SecretVar{}
	if password != nil {
		passwordSecret = schemas.NewSecretVar(*password)
	}
	return &AuthConfig{
		AdminUserName: usernameSecret,
		AdminPassword: passwordSecret,
		IsEnabled:     isEnabled,
...
func (s *RDBConfigStore) UpdateAuthConfig(ctx context.Context, config *AuthConfig) error {
	...
	return s.DB().WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Save(&tables.TableGovernanceConfig{
			...
			Value: secretVarToStoredString(config.AdminUserName),
		}).Error; err != nil {
			return err
		}
		if err := tx.Save(&tables.TableGovernanceConfig{
			Key:   tables.ConfigAdminPasswordKey,
			Value: secretVarToStoredString(config.AdminPassword),
		}).Error; err != nil {
			return err
		}
		if err := tx.Save(&tables.TableGovernanceConfig{
		...
		if err := tx.Save(&tables.TableGovernanceConfig{
			Key:   tables.ConfigAuthTypeKey,
			Value: authType,
		}).Error; err != nil {
			return err
		}
		if config.OIDC == nil {
			return tx.Delete(&tables.TableGovernanceConfig{}, "key = ?", tables.ConfigOIDCAuthKey).Error
		}
		oidcConfig := storedOIDCAuthConfig{
			IssuerURL:           config.OIDC.IssuerURL,
			ClientID:            config.OIDC.ClientID,
			ClientSecret:        secretVarToStoredString(config.OIDC.ClientSecret),
			RedirectURL:         config.OIDC.RedirectURL,
			Scopes:              slices.Clone(config.OIDC.Scopes),
			AllowedEmailDomains: slices.Clone(config.OIDC.AllowedEmailDomains),
			AllowedEmails:       slices.Clone(config.OIDC.AllowedEmails),
		}
		data, err := json.Marshal(&oidcConfig)
		if err != nil {
			return fmt.Errorf("failed to marshal OIDC auth config: %w", err)
		}
		if err := tx.Save(&tables.TableGovernanceConfig{
			Key:   tables.ConfigOIDCAuthKey,
			Value: string(data),
		}).Error; err != nil {
			return err
		}
