...
func (s *BifrostHTTPServer) UpdateAuthConfig(ctx context.Context, authConfig *configstore.AuthConfig) error {
	...
	// Allow disabling auth without credentials, but require the selected auth
	// mode's credentials when enabling.
	switch authConfig.EffectiveAuthType() {
	case configstore.AuthTypeNone:
	case configstore.AuthTypeOIDC:
		if authConfig.OIDC == nil || !authConfig.OIDC.IsConfigured() {
			return fmt.Errorf("OIDC issuer_url, client_id, and client_secret are required when OIDC auth is enabled")
		}
	default:
		if authConfig.AdminUserName == nil || authConfig.AdminUserName.GetValue() == "" || authConfig.AdminPassword == nil || authConfig.AdminPassword.GetValue() == "" {
