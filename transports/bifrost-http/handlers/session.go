...
const dashboardSessionTTL = time.Hour * 24 * 30
...
func (h *SessionHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.GET("/api/session/sso/start", lib.ChainMiddlewares(h.startOIDCLogin, middlewares...))
	r.GET("/api/session/sso/callback", lib.ChainMiddlewares(h.completeOIDCLogin, middlewares...))
...
func dashboardAuthType(authConfig *configstore.AuthConfig) string {
	if authConfig == nil {
		return configstore.AuthTypeNone
	}
	return authConfig.EffectiveAuthType()
}
...
func (h *SessionHandler) login(ctx *fasthttp.RequestCtx) {
	...
	if dashboardAuthType(authConfig) == configstore.AuthTypeOIDC {
		SendError(ctx, fasthttp.StatusForbidden, "Password login is disabled; use SSO")
		return
	}
	...
	token, expiresAt, err := h.createDashboardSession(ctx)
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to create session: %v", err))
		return
	}
	setDashboardSessionCookie(ctx, token, expiresAt)
...
func (h *SessionHandler) createDashboardSession(ctx *fasthttp.RequestCtx) (string, time.Time, error) {
	now := time.Now()
...
func setDashboardSessionCookie(ctx *fasthttp.RequestCtx, token string, expiresAt time.Time) {
...
func isHTTPSRequest(ctx *fasthttp.RequestCtx) bool {
	return string(ctx.Request.Header.Peek("X-Forwarded-Proto")) == "https" || ctx.IsTLS()
}
