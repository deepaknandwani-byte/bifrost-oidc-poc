...
func (h *ConfigHandler) getConfig(ctx *fasthttp.RequestCtx) {
	...
	if h.store.ConfigStore != nil {
		...
		if authConfig != nil {
			...
			usernameSecretVar := authConfig.AdminUserName
			if usernameSecretVar == nil {
				usernameSecretVar = &schemas.SecretVar{}
			}
			...
			redactedAuthConfig := map[string]any{
				"admin_username": usernameSecretVar,
				"admin_password": passwordSecretVar,
				"is_enabled":     authConfig.IsEnabled,
				"auth_type":      authConfig.EffectiveAuthType(),
			}
			if authConfig.OIDC != nil {
				redactedAuthConfig["oidc"] = authConfig.OIDC.FullyRedacted()
			}
			mapConfig["auth_config"] = redactedAuthConfig
		}
...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	if payload.AuthConfig != nil {
		...
		if authConfig == nil {
			...
			authTypeChanged := payload.AuthConfig.EffectiveAuthType() != authConfig.EffectiveAuthType()
			oidcChanged := !reflect.DeepEqual(payload.AuthConfig.OIDC, authConfig.OIDC)
		...
		if payload.AuthConfig.IsEnabled {
			if payload.AuthConfig.EffectiveAuthType() == configstore.AuthTypeOIDC {
				if payload.AuthConfig.OIDC == nil || !payload.AuthConfig.OIDC.IsConfigured() {
					SendError(ctx, fasthttp.StatusBadRequest, "OIDC issuer_url, client_id, and client_secret must be provided")
					return
				}
				if payload.AuthConfig.OIDC.ClientSecret.IsFromSecret() && payload.AuthConfig.OIDC.ClientSecret.GetValue() == "" {
					SendError(ctx, fasthttp.StatusBadRequest, fmt.Sprintf("external reference %s for oidc.client_secret resolved to an empty value", payload.AuthConfig.OIDC.ClientSecret.GetRawRef()))
					return
				}
			...
			if payload.AuthConfig.AuthType == "" {
				payload.AuthConfig.AuthType = authConfig.AuthType
			}
