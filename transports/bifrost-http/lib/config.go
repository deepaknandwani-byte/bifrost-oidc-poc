...
func loadAuthConfig(ctx context.Context, config *Config, configData *ConfigData) {
	...
	if authConfig.EffectiveAuthType() == configstore.AuthTypeOIDC {
		if authConfig.OIDC != nil && authConfig.OIDC.ClientSecret != nil &&
			authConfig.OIDC.ClientSecret.IsFromSecret() && authConfig.OIDC.ClientSecret.GetValue() == "" {
			logger.Warn("OIDC client_secret set with external reference but value is empty: %s", authConfig.OIDC.ClientSecret.GetRawRef())
		}
		if authConfig.OIDC == nil || !authConfig.OIDC.IsConfigured() {
			logger.Warn("OIDC auth config is missing issuer_url, client_id, or client_secret, skipping auth config processing")
			return
		}
		if dbAuthConfig != nil && dashboardOIDCAuthConfigMatches(dbAuthConfig, authConfig) {
			config.GovernanceConfig.AuthConfig = dbAuthConfig
			return
		}
		if dbAuthConfig != nil {
...
func dashboardOIDCAuthConfigMatches(dbAuthConfig *configstore.AuthConfig, fileAuthConfig *configstore.AuthConfig) bool {
	if dbAuthConfig == nil || fileAuthConfig == nil {
		return dbAuthConfig == fileAuthConfig
	}
	return dbAuthConfig.IsEnabled == fileAuthConfig.IsEnabled &&
		dbAuthConfig.EffectiveAuthType() == fileAuthConfig.EffectiveAuthType() &&
		reflect.DeepEqual(dbAuthConfig.OIDC, fileAuthConfig.OIDC)
}
