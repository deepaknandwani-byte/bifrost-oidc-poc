...
function getPortConfig(): PortConfig {
	...
	if (isDevelopment) {
		// When the dev UI is reached through the Go server (including ngrok),
		// keep API/SSO calls same-origin so cookies land on the callback host.
		...
		// Direct Vite development: UI runs on 3000 and talks to Go on 8080.
